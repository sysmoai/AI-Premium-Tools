  ...
  const NEXT_STEPS = [
    {
      ...
      time: "Within 30 min",
      desc: "We verify your payment transaction",
    ...
    },
    {
      ...
      time: "Within 1 hour",
      desc: "You receive access credentials via WhatsApp",
    ...
    },
    {
      ...
      time: "Immediately",
      desc: "Start using your premium AI tools",
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-16 text-center">
      ...
      <div className="relative inline-flex items-center justify-center mb-8">
        <div
          className="absolute h-28 w-28 rounded-full opacity-30 pulse-ring"
          style={{ background: "radial-gradient(circle, hsl(142 76% 50%), transparent)" }}
        />
        <div className="relative h-24 w-24 rounded-full flex items-center justify-center shadow-xl"
      ...
      <p className="text-muted-foreground text-lg mb-10">
        Thank you! We'll verify your payment and deliver your AI tools within 1 hour.
      </p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <div className="font-bold text-xl font-mono" data-testid="text-order-id">#{order.id}</div>
              </div>
              <Badge className={`${STATUS_COLORS[order.status] || ""} border-0 capitalize text-sm px-3 py-1`} data-testid="badge-status">
                {order.status}
              </Badge>
            </div>
            ...
            <div className="border-t pt-4 flex justify-between font-bold text-xl mb-4">
              <span>Total</span>
            ...
            <div className="pt-4 border-t grid grid-cols-2 gap-4 text-sm">
              <div>
                <div className="text-muted-foreground text-xs mb-0.5">Payment Method</div>
                <div className="font-medium capitalize">{order.payment_method?.replace("_", " ")}</div>
              </div>
              {order.payment_ref && (
                <div>
                  <div className="text-muted-foreground text-xs mb-0.5">Ref No.</div>
                  <div className="font-medium font-mono">{order.payment_ref}</div>
                </div>
              )}
              {order.customer_name && (
                <div>
                  <div className="text-muted-foreground text-xs mb-0.5">Customer</div>
      ...
      <div className="text-left mb-10">
        ...
        <div className="space-y-3">
          {NEXT_STEPS.map((step, idx) => (
            <div
              key={idx}
              className={`flex items-center gap-4 p-4 rounded-lg border ${step.color} ${step.bg}`}
            >
              <div className="h-10 w-10 rounded-full bg-white dark:bg-black/20 flex items-center justify-center shadow-sm shrink-0">
                {step.icon}
              </div>
              <div>
                <div className="font-semibold text-sm">{step.time}</div>
      ...
      <div className="flex flex-wrap gap-4 justify-center">
        <Link href="/products">
          <Button variant="outline" className="rounded-full px-8 h-12" data-testid="btn-continue-shopping">
            Continue Shopping
          </Button>
        </Link>
        <Link href="/">
          <Button className="rounded-full px-8 h-12 font-bold shadow-md" data-testid="btn-back-home">
